...
export default function Progress() {
  ...
  const weeklyBars = useMemo(() => {
    return Array.from({ length: 7 }, (_, index) => {
      ...
      return {
        ...
        completed: dayItems.filter(isCompleted).length
      };
    });
  }, [items]);
  ...
  const maxWeekly = Math.max(...weeklyBars.map((d) => d.count), 1);
    ...
    <main className="progressThemePage min-h-screen bg-gradient-to-br from-sky-50 via-white to-cyan-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="progressHero overflow-hidden rounded-[34px] bg-gradient-to-r from-sky-600 via-cyan-500 to-sky-400 p-8 text-white shadow-xl shadow-sky-200">
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm font-semibold text-sky-50">
                Real-time daily progress from Today Plan, DSA Tracker, Practice MCQs, Resume Coach and Interview Coach.
              </p>
            </div>

            <div className="rounded-[28px] bg-white/15 p-5 ring-1 ring-white/25 backdrop-blur">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-sky-50">
                Readiness
              </p>
              <h2 className="mt-2 text-5xl font-black">{readiness}%</h2>
              <div className="mt-4 h-3 w-56 overflow-hidden rounded-full bg-white/25">
                <div
        ...
        <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-lg shadow-sky-50 ring-1 ring-sky-100">
            <div>
              <h2 className="text-xl font-black text-slate-900">Last 7 Days Activity</h2>
              <p className="mt-1 text-sm font-semibold text-slate-500">
                Real weekly activity trend from backend progress data.
              </p>
            </div>

            <div className="mt-6 grid grid-cols-7 gap-3">
              {weeklyBars.map((day) => {
                const height = Math.max(14, Math.round((day.count / maxWeekly) * 150));

                return (
                  <div key={day.label} className="flex flex-col items-center justify-end gap-2">
                    <div className="flex h-40 items-end">
                      <div
                        className="w-8 rounded-t-2xl bg-gradient-to-t from-sky-500 to-emerald-300 shadow-md shadow-sky-100"
                        style={{ height }}
                        title={`${day.count} activities`}
                      />
                    </div>
                    <p className="text-xs font-black text-slate-500">{day.label}</p>
            ...
            <div className="mt-6 rounded-2xl bg-sky-50 p-4 ring-1 ring-sky-100">
              <p className="text-sm font-bold text-slate-600">
                Auto-refresh is active every 5 seconds. Any new saved activity will appear here without manual page reload.
              </p>
            </div>
          </div>
        </section>
