  ...
  return (
    <main className="practiceMcqThemePage min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-white p-6">
      <div className="practiceMcqShell mx-auto max-w-7xl space-y-6">
        <section className="practiceMcqHero rounded-[34px] bg-gradient-to-r from-indigo-700 via-violet-700 to-slate-950 p-8 text-white shadow-xl shadow-indigo-200">
        ...
        <section className="practiceMcqFilter rounded-[30px] bg-white p-5 shadow-lg shadow-indigo-50 ring-1 ring-indigo-100">
        ...
        <section className="practiceMcqSet rounded-[30px] bg-white p-6 shadow-lg shadow-indigo-50 ring-1 ring-indigo-100">
          ...
          <div className="practiceMcqGrid mt-6 space-y-5">
            ...
            {paginatedQuestions.map((q, index) => {
              ...
              const selectedKey = checked?.selected || selectedAnswers[q.id];

              return (
                <div key={q.id} className="practiceMcqCard rounded-[26px] border border-slate-200 bg-slate-50 p-5">
                    ...
                    {(q.options || []).map((option) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            if (checked) return;
                            setSelectedAnswers((prev) => ({ ...prev, [q.id]: option.key }));
                          }}
                  ...
                  {!checked && (
                    <button
                      ...
                      onClick={() => chooseAnswer(q.id, selectedAnswers[q.id])}
                      className="mcqSubmitAnswerBtn"
                    >
                      Submit Answer
                    </button>
                  )}
          ...
          {questions.length > pageSize && (
            <div className="practiceMcqPagination mt-8 flex flex-wrap items-center justify-between gap-4 rounded-[24px] bg-slate-50 p-4 ring-1 ring-slate-200">
              <button
