...
export default function RolePractice() {
  ...
  async function fetchQuestionUrl(url) {
    try {
  ...
  async function loadQuestions() {
    ...
    try {
      ...
      for (const candidate of aliases) {
        ...
        urls.push(`${API_BASE}/api/practice/questions?track=${encoded}&limit=200`);
      }
      ...
      for (const url of urls) {
        const list = await fetchQuestionUrl(url);
