...
function StatCard({ label, value }) {
  return (
    <div className="rounded-3xl border border-white/15 bg-white/10 p-5 backdrop-blur-xl">
      <p className="text-xs font-black uppercase tracking-wide text-indigo-100">{label}</p>
...
function skillInResume(skill, resumeText) {
  ...
  const aliases = {
    ...
    oops: ["oops", "object oriented", "object-oriented", "oop"],
  ...
  return checks.some((word) => {
    const w = cleanText(word);
    if (!w) return false;
    return resumeClean.includes(w);
  });
}
...
export default function ResumeCoach() {
  ...
  const datasetRoleSkills = useMemo(() => {
    return uniqueList(selectedRoleData?.skills || selectedRoleData?.requiredSkills || []);
  }, [selectedRoleData]);

  const strictMatchedSkills = useMemo(() => {
    const backendMatched = analysis?.matchedSkills || [];
    const datasetMatched = datasetRoleSkills.filter((skill) =>
      skillInResume(skill, resumeText)
    );

    return uniqueList([...backendMatched, ...datasetMatched]);
  }, [analysis, datasetRoleSkills, resumeText]);

  const strictMissingSkills = useMemo(() => {
    const backendMissing = analysis?.missingSkills || [];
    const datasetMissing = datasetRoleSkills.filter((skill) =>
      !skillInResume(skill, resumeText)
    );

    return uniqueList([...datasetMissing, ...backendMissing]).filter(
      (skill) => !strictMatchedSkills.some((matched) => cleanText(matched) === cleanText(skill))
    );
  }, [analysis, datasetRoleSkills, resumeText, strictMatchedSkills]);

  const strictRoleMatchScore = useMemo(() => {
    if (!analysis) return "--";

    if (!datasetRoleSkills.length) return `${analysis.roleMatchScore}%`;

    const score = Math.round((strictMatchedSkills.length / datasetRoleSkills.length) * 100);
    return `${Math.max(0, Math.min(100, score))}%`;
  }, [analysis, datasetRoleSkills, strictMatchedSkills]);
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-slate-50 via-indigo-50/60 to-violet-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto w-full max-w-[1500px] space-y-7">
        <section className="relative overflow-hidden rounded-[38px] bg-gradient-to-br from-[#0f172a] via-[#312e81] to-[#6d28d9] p-10 text-white shadow-2xl shadow-indigo-200 sm:p-10">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-3 max-w-3xl text-sm font-semibold leading-6 text-indigo-100 sm:text-base">
              Strict resume check for company roles.
            </p>          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-slate-500">
                  Search company, select company, then choose a role from the styled dropdown.
                </p>
              </div>
          ...
          <div className="rounded-[34px] bg-white/95 p-6 shadow-xl shadow-indigo-50 ring-1 ring-indigo-100 backdrop-blur">
            ...
            <div className="mt-4 rounded-[24px] border border-indigo-100 bg-indigo-50 p-4">
              ...
              <p className="mt-1 text-xs font-bold leading-5 text-indigo-700">
                It compares the complete resume text with the selected role skills from the dataset and highlights actually missing skills.
              </p>
            </div>
        ...
        {analysis && (
          <section className="space-y-6">
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-[34px] bg-white/95 p-6 shadow-xl shadow-indigo-50 ring-1 ring-indigo-100">
                <h2 className="text-2xl font-black text-slate-900">Strict Missing Skills</h2>
                <p className="mt-1 text-sm font-semibold text-slate-500">
                  Required by selected role dataset but not found in your resume text.
                </p>
                <div className="mt-4 flex flex-wrap gap-2">
            ...
            <div className="rounded-[34px] bg-white/95 p-6 shadow-xl shadow-indigo-50 ring-1 ring-indigo-100">
              <h2 className="text-2xl font-black text-slate-900">Weak Resume Points</h2>
              <div className="mt-5 space-y-4">
                {analysis.weakPoints?.length ? (
                  analysis.weakPoints.map((item, index) => (
                    <div key={index} className="rounded-[24px] border border-amber-200 bg-amber-50 p-5">
                      <p className="text-sm font-black text-amber-900">Weak line:</p>
                      <p className="mt-1 text-sm font-semibold text-amber-800">{item.weakLine}</p>
                      <p className="mt-3 text-sm font-black text-amber-900">Issue:</p>
                      <p className="mt-1 text-sm font-semibold text-amber-800">{item.issue}</p>
                      <p className="mt-3 text-sm font-black text-green-900">Improved version:</p>
            ...
            <div className="rounded-[34px] bg-white/95 p-6 shadow-xl shadow-indigo-50 ring-1 ring-indigo-100">
              <h2 className="text-2xl font-black text-slate-900">Suggested Projects</h2>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {analysis.projectSuggestions?.map((project, index) => (
                  <div key={index} className="rounded-[24px] border border-indigo-100 bg-gradient-to-br from-white to-indigo-50/60 p-5">
                    <h3 className="text-lg font-black text-slate-900">{project.title}</h3>
                    <p className="mt-3 text-sm font-bold text-slate-600">{project.whatToBuild}</p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {(project.skillsCovered || []).map((skill, i) => (
                        <Badge key={i} type="primary">{skill}</Badge>
                      ))}
                    </div>
                    <div className="mt-4 rounded-2xl bg-white p-4 text-sm font-semibold text-slate-700 ring-1 ring-slate-200">
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm font-bold text-amber-800">
              {analysis.disclaimer || "This analysis is based on resume text and selected role skills from the dataset."}
            </div>
          </section>
        )}
      </div>
