...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#eef4fb] px-6 py-10">
      <div className="mx-auto grid min-h-[calc(100vh-80px)] max-w-6xl items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="hidden rounded-[2.5rem] bg-gradient-to-br from-slate-950 via-indigo-900 to-blue-600 p-10 text-white shadow-2xl lg:block">
          ...
          <p className="mt-5 max-w-xl text-base font-semibold leading-7 text-indigo-100">
            Track DSA, practice MCQs, improve your resume, and prepare interviews with a focused dashboard.
          </p>

          <div className="mt-10 grid grid-cols-2 gap-4">
            {["DSA Tracker", "Resume Coach", "Interview Coach", "Company Prep"].map((item) => (
              <div key={item} className="rounded-2xl bg-white/10 p-5 ring-1 ring-white/20">
                <p className="text-sm font-black">{item}</p>
                <p className="mt-1 text-xs font-bold text-indigo-100">Ready</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-[2rem] bg-white p-8 shadow-xl ring-1 ring-slate-200">
          ...
          <p className="mt-2 text-center text-sm font-semibold text-slate-500">
            Login to open your SkillYatra dashboard.
          </p>
          ...
          <p className="mt-6 text-center text-sm font-semibold text-slate-500">
            New user?{" "}
            <Link to="/signup" className="font-black text-indigo-600">
              Create account
            </Link>
          </p>
