...
export default function RolePractice() {
  ...
  const { roleName, role } = useParams();
  const strictSelectedRole = decodeURIComponent(roleName || role || "").trim();
  const STRICT_QUESTIONS_PER_PAGE = 25;
  const [strictPage, setStrictPage] = useState(1);
...
const params = useParams();
  ...
  const totalQuestions = Array.isArray(filteredQuestions) ? filteredQuestions.length : 0;
  ...
  const paginatedQuestions = Array.isArray(filteredQuestions)
    ? filteredQuestions.slice(startIndex, startIndex + QUESTIONS_PER_PAGE)
    : [];


  const strictRoleQuestions = useMemo(() => {
    const list = Array.isArray(paginatedQuestions) ? paginatedQuestions : [];
    const selected = String(strictSelectedRole || "").toLowerCase().trim();
    ...
    return list.filter((q) => {
      ...
      return (
        roleValue === selectedNorm ||
        roleValue.includes(selectedNorm) ||
        selectedNorm.includes(roleValue)
      );
    });
  }, [paginatedQuestions, strictSelectedRole]);

  const strictTotalQuestions = strictRoleQuestions.length;
  const strictTotalPages = Math.max(1, Math.ceil(strictTotalQuestions / STRICT_QUESTIONS_PER_PAGE));
  const strictSafePage = Math.min(strictPage, strictTotalPages);
  const strictStartIndex = (strictSafePage - 1) * STRICT_QUESTIONS_PER_PAGE;
  const strictPaginatedQuestions = strictRoleQuestions.slice(
    strictStartIndex,
    strictStartIndex + STRICT_QUESTIONS_PER_PAGE
  );

return (
    <div className="sy-page-theme sy-role-practice-theme min-h-screen bg-[#eef4fb] px-8 py-8">
      ...
      <section className="mt-8 rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="grid gap-4 lg:grid-cols-[1fr_220px_150px]">
          <div className="practiceSearchBarFinal flex h-12 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-50">
          ...
          <select className="practiceSelectExact practiceSelectExact"
            value={difficulty}
          ...
          <button
            ...
            className="practiceRefreshExact inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-black text-white shadow-lg shadow-indigo-100 hover:bg-indigo-700"
          >
      ...
      <section className="mt-8">
        {loading ? (
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {strictPaginatedQuestions.map((item, index) => {
              const selected = answers[item.id];
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <button
                      ...
                      className="practice-submit-answer-btn rounded-xl bg-indigo-600 px-4 py-2 text-sm font-black text-white hover:bg-indigo-700"
                    >
        ...
        {totalQuestions > QUESTIONS_PER_PAGE && (
          <div className="rolePracticePagination">
            <button
              type="button"
              onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}
              disabled={safePage === 1}
              className="rolePracticePageBtn"
            >
              ← Previous
            </button>

            <div className="rolePracticePageInfo">
              Page {safePage} of {totalPages}
              <span>
                Showing {startIndex + 1}-{Math.min(startIndex + QUESTIONS_PER_PAGE, totalQuestions)} of {totalQuestions} questions
              </span>
            </div>
        ...
        {strictTotalQuestions > STRICT_QUESTIONS_PER_PAGE && (
          <div className="strictPracticePagination">
            <button
              type="button"
              onClick={() => setStrictPage((page) => Math.max(1, page - 1))}
              disabled={strictSafePage === 1}
              className="strictPracticePageBtn"
            >
              ← Previous
            </button>

            <div className="strictPracticePageInfo">
              Page {strictSafePage} of {strictTotalPages}
              <span>
                Showing {strictStartIndex + 1}-{Math.min(strictStartIndex + STRICT_QUESTIONS_PER_PAGE, strictTotalQuestions)} of {strictTotalQuestions} role-wise questions
              </span>
            </div>

            <button
              type="button"
              onClick={() => setStrictPage((page) => Math.min(strictTotalPages, page + 1))}
              disabled={strictSafePage === strictTotalPages}
              className="strictPracticePageBtn"
            >
              Next →
            </button>
