...
def detect_platform(row, title):
    raw = " ".join([
        ...
        pick(row, ["url", "URL", "link", "Link", "problem_link", "problem url", "question link", "Problem Link", "problem_url"]),
    ...
    if "codeforces" in raw or "code forces" in raw:
        return "Codeforces"
    if "geeksforgeeks" in raw or "geeks for geeks" in raw or "gfg" in raw:
    ...
    if "codeforces" in src:
        return "Codeforces"
    if "gfg" in src or "geeksforgeeks" in src or "geeks for geeks" in src:
        return "GeeksforGeeks"
...
def is_coding_question(row, title):
    ...
    if "gfg" in raw or "geeksforgeeks" in raw or "geeks for geeks" in raw:
        return not any(bad in raw for bad in BAD_QUESTION_WORDS)

    return any(sig in raw for sig in CODING_SIGNALS)

def direct_url(row, title, plat):
    direct = pick(row, ["url", "URL", "link", "Link", "problem_link", "problem url", "question link", "Problem Link", "problem_url"])
    ...
    if plat == "GeeksforGeeks":
        direct = pick(row, ["url", "URL", "link", "Link", "Problem Link", "problem_link", "problem_url", "question link"])
        if direct.startswith("http"):
            return direct
...
for file in files:
    ...
    for idx, row in enumerate(rows):
        raw_title = pick(row, [
            ...
            "titleSlug", "title_slug", "slug", "problem_name", "problemName", "problem_title", "Problem Name", "Problem Title"
        ])
